...
type Options struct {
	...
	Name       string
	Dir        string
	MuxSession string // prefix for the derived tmux session name
...
func (o Options) spec() (mux.Spec, string, error) {
	id, sess := o.resolveSession()
	inner, err := o.inner(id, sess)
	if err != nil {
		return mux.Spec{}, "", err
	}
	return mux.Spec{ID: id, Name: o.Name, Dir: o.Dir, Session: sess, Socket: o.MuxSocket, InnerArgv: inner}, sess, nil
}
...
// Command returns the tmux argv without running it (dry-run).
func Command(o Options) ([]string, error) {
	spec, _, err := o.spec()
	if err != nil {
		return nil, err
	}
	return mux.Command(spec)
}

// Launch creates the session's tmux server and starts the wrapper in it.
func Launch(o Options) (string, error) {
	spec, sess, err := o.spec()
	if err != nil {
		return "", err
	}
	return sess, mux.Launch(spec)
}
